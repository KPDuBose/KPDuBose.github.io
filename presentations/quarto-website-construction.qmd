...
Additionally, we can add a category label to the pages in our files and a filter based on topics. 
...
All that changes from the previous section is the addition of the `categories`. By using the `true` option, the sorting is numbered. All the categories are listed alphanumerically (I think) and lists the number of pages in each category. There are other options, such as an `unnumbered` or a `cloud` option, but I prefer the default.

After this is updated, each page needs to have the `categories` option as well. It should look like:

```
categories: ["First category", "Maybe a second", "Look a third"]
```

Once those have been added, run `quarto render` from the terminal to reconstruct the website and use `quarto preview` to view it. (When I ran just `quarto preview`, it didn't always update the categories correctly.)

## That's it, for now

That should be enough information to help you get started. Quarto is a versatile tool, and they have a lot of tutorials to help one get proficient at using it. 

Feel free to check out my [GitHub project](https://github.com/KPDuBose/KPDuBose.github.io/tree/master) and let me know if you have any questions. 
